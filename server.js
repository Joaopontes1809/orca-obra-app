...
app.patch('/api/pedidos/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const allowed = ['nome_cliente', 'telefone', 'morada', 'tipo_servico', 'descricao', 'quantidade', 'unidade', 'status', 'nome_orcamento', 'observacoes_internas', 'itens'];
    const fields = [];
    const values = [];
    let i = 1;
    for (const key of allowed) {
      if (Object.prototype.hasOwnProperty.call(req.body, key)) {
        fields.push(`${key} = $${i}`);
        values.push(key === 'itens' ? JSON.stringify(req.body[key]) : req.body[key]);
        i++;
      }
    }
...
initDb()
  .then(() => {
    app.listen(port, () => console.log('Orça Obra a correr na porta ' + port));
  })
  .catch(e => {
